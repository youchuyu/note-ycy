...
import { WorldAPM } from '@/src/bizComponents/parallelWorld/constants';
import { reportClick } from '@/src/utils/report';
import { AnimationDir } from '../../../components/MaskImages';
import {
  ActDialog,
  ActItem,
  ActStory,
...
export const useDisplayMaskCardVideoInit = ({
  ...
  useEffect(() => {
    if (showVideo) {
      if (videoText) {
        $firstImgTextOpacity.value = withDelay(1000, withTiming(1));
      } else {
        handleCloseVideo();
        // setTimeout(() => {
        //   handleCloseVideo();
  ...
  return { isVideoPlayed, handleCloseVideo };
};
...
export const useImgAnimatedStyle = ({
  ...
  storyList: PartialMessage<ActItem>[];
