...
const editBtnText: Record<
  string,
  { text: '编辑' | '保存' | '撤销'; icon: IconProps['icon'] }
> = {
  edit: {
    text: '编辑',
    icon: 'icon_edit_pw'
...
export default function ConsumerHeaderRight() {
  ...
  if (
    isNextChapterModalVisible ||
    uid !== newTimeLine[activeTimelineSectionIdx]?.author?.uid
  ) {
    return null;
  }
    ...
    <View style={[$flexHCenter, { gap: 14 }]}>
      {isGenCardEditable ? (
        <>
          ...
          >
            <Icon icon={editBtnText.cancel.icon} size={16} />
            <Text
              style={{
          ...
          <ParallelWorldButton
            ...
            onPress={handleSave}
          >
            <Icon icon={editBtnText.save.icon} size={16} />
            <Text
              style={{
                lineHeight: BUTTON_HEIGHT,
                marginLeft: 4,
        ...
        <>
          <ParallelWorldButton
            ...
            onPress={handleEdit}
          >
            <Icon icon={editBtnText.edit.icon} size={16} />
            <Text
              style={{
                lineHeight: BUTTON_HEIGHT,
                marginLeft: 4,
