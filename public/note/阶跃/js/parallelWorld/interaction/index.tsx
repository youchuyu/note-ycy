...
import { BUTTON_HEIGHT } from '@/src/bizComponents/parallelWorld/constants';
...
import { Scene } from '@Components/comment/typing';
import { useShallow } from 'zustand/react/shallow';

export function InteractionBottom({ detailId }: { detailId: string }) {
  const { topCommentId, showCommentInput } = useLocalSearchParams();
  ...
  // useEffect(() => {
  //   log.log('ewewew', { commonInfo, detailId });
  // }, [commonInfo, detailId]);
  const { world, topic } = useWorldStore(
    useShallow(state => ({
      world: state.currentWorld?.world,
      topic: state.currentWorld?.world?.topic
    }))
  );
  ...
  const onPressBtn = () => {
    reportClick(
      'buttombar_button',
      { contentid: detailId, module: 'world' },
      'comment'
    );
    setVisible(true);
  ...
  const clickHandler = (isLike: boolean) => {
    reportClick(
      'buttombar_button',
      { contentid: detailId, isLike, module: 'world' },
      'like'
    );
  };
  ...
  useEffect(() => {
    // 当有置顶评论/拍同款评论时，自动打开评论弹窗
    if (detailId && (topCommentId || showCommentInput)) {
      setVisible(true);
    }
  }, [detailId, showCommentInput]);
      ...
      <Pressable onPress={onPressBtn}>
        <View style={$commentIconContainer}>
          ...
          <CommentModal
            ...
            portalHostName={`CommentPortalHost_${detailId ?? ''}`}
            headerSlot={
              topic ? (
